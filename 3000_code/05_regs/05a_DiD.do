...
capture log close
log using "3000_code/05_regs/05_SMCL_logs/05a_DiD", smcl replace
...
save "2000_data\500_working\14_days\eq1\05a_use_in_loop.dta", replace
...
foreach o in bstop_share bsearch_share bhit_share bsearch_rate bhit_rate bhit_bstop_ratio n_stops n_bstops n_searches n_bsearches n_hits n_bhits {
	...
	use "2000_data\500_working\14_days\eq1\05a_use_in_loop.dta", replace
	...
	* Setting OUTCOME DESCRIPTIONS: 
	...
	display ""
	display "*********************************************************************"
	display "Eq1: DiD, outcome=""`o'"": `o_desc'"
	display "*********************************************************************"
	...
	* running regressions: 
	...
	reghdfe `o' post0 `aweight' if (date>1) `ifs', a(`i_absorb') cl(num_dept) base resid 
	est save "4000_outputs/`k'_days/eq1/`o'/DiD_`variant'`suffix'", replace
	...
	* Using balanced panel for the rest: 
	...
	reghdfe `o' post0 `aweight' if (date>1) `ifs1', a(`i_absorb') cl(num_dept) base resid 
	est save "4000_outputs/`k'_days/eq1/`o'/DiD_`variant'`suffix'", replace
	gen e_`o'_`variant' = (e(sample)==1)
	...
	reghdfe `o' post0 `aweight' if (date>1) `ifs1', a(`i_absorb2') cl(num_dept) base resid 
	est save "4000_outputs/`k'_days/eq1/`o'/DiD_`variant'`suffix'", replace
	gen e_`o'_`variant' = (e(sample)==1)
	...
	reghdfe `o' post0 `aweight' if (date>1) `ifs1', a(`i_absorb3') cl(num_dept) base resid
	est save "4000_outputs/`k'_days/eq1/`o'/DiD_`variant'`suffix'", replace
	gen e_`o'_`variant' = (e(sample)==1)
	...
	if ("`o'" == "n_stops") | ("`o'" == "n_bstops") | ("`o'" == "n_searches") | ("`o'" == "n_bsearches") | ("`o'" == "n_hits") | ("`o'" == "n_bhits") {
		...
		reghdfe log_`o' post0 `aweight' if (date>1) `ifs1', a(`i_absorb') cl(num_dept) base resid 
		est save "4000_outputs/`k'_days/eq1/`o'/DiD_`variant'`suffix'", replace
		gen e_`o'_`variant' = (e(sample)==1)
	}
	...
	reghdfe `o' post0 `aweight' if (multi_dept==0) `ifs1', a(`i_absorb') cl(num_dept) base resid 
	est save "4000_outputs/`k'_days/eq1/`o'/DiD_`variant'`suffix'", replace
	gen e_`o'_`variant' = (e(sample)==1)
	...
	reghdfe `o' post0 `aweight' if (multi_dept==1) `ifs1', a(`i_absorb') cl(num_dept) base resid 
	est save "4000_outputs/`k'_days/eq1/`o'/DiD_`variant'`suffix'", replace
	gen e_`o'_`variant' = (e(sample)==1)
	...
	reghdfe `o' post0 `aweight' if (statewide==1) `ifs1', a(`i_absorb') cl(num_dept) base resid 
	est save "4000_outputs/`k'_days/eq1/`o'/DiD_`variant'`suffix'", replace
	gen e_`o'_`variant' = (e(sample)==1)
	...
	reghdfe `o' post0 `aweight' if (statewide==0) `ifs1', a(`i_absorb') cl(num_dept) base resid 
	est save "4000_outputs/`k'_days/eq1/`o'/DiD_`variant'`suffix'", replace
	gen e_`o'_`variant' = (e(sample)==1)
	...
	* joint F-test, to see if new interactions are relevant: 
	...
	save "2000_data/500_working/`k'_days/eq1/`o'/e_DiD`suffix'.dta", replace
}

* merging-together all 12 outcome datasets (useful for descriptive statistics table)
use "2000_data/500_working/`k'_days/eq1/n_stops/e_DiD`suffix'.dta", replace
foreach o in n_bstops n_searches n_bsearches n_hits n_bhits bstop_share bsearch_share bsearch_rate bhit_share bhit_rate bhit_bstop_ratio {
	merge 1:1 num_dept date using "2000_data/500_working/`k'_days/eq1/`o'/e_DiD`suffix'.dta", nogen
}

save "2000_data/500_working/`k'_days/eq1/e_DiD`suffix'.dta", replace
